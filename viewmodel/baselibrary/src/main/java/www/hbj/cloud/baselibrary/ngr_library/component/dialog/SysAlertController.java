...
public class SysAlertController {
	...
	private void setupView() {
		...
		TypedArray a = mContext.obtainStyledAttributes(null, R.styleable.HerilyAlertDialog,
				R.style.alertDialogStyle, 0);
	...
	private boolean setupTitle(LinearLayout topPanel) {
		...
		if (mCustomTitleView != null) { //有自定义title则隐藏默认的title
			...
			if (hasTextTitle) {
				...
				/*
				 * Do this last so that if the user has supplied any icons we use them
				 * instead of the default ones. If the user has specified 0 then make it disappear.
				 */
				if (mIconView != null) {
					if (mIconId > 0) {
						mIconView.setImageResource(mIconId);
	...
	private boolean setupButtons() {
		...
		switch (whichButtons) { //通常情况只使用两个按钮，其中一个是POSITIVE
			case 7://Positive、Neutral、Negative
				...
				mButtonPositive.setBackgroundDrawable(mContext.getResources().getDrawable(
						R.drawable.herily_alertex_dlg_btn_left_drawable));
				mButtonNeutral.setBackgroundDrawable(mContext.getResources().getDrawable(
						R.drawable.herily_alertex_dlg_btn_center_drawable));
				mButtonNegative.setBackgroundDrawable(mContext.getResources().getDrawable(
						R.drawable.herily_alertex_dlg_btn_right_bdrawable));
				break;
			...
			case 1://Positive	[一个按钮则隐藏竖向分割线]
				...
				mWindow.findViewById(R.id.btn_middle_divider).setVisibility(View.GONE);
				mButtonPositive.setBackgroundDrawable(mContext.getResources().getDrawable(
						R.drawable.herily_alertex_dlg_btn_singlebtn_drawable));
				break;
			case 2://Negative
				mWindow.findViewById(R.id.btn_middle_divider).setVisibility(View.GONE);
				mWindow.findViewById(R.id.btn_middle_divider).setVisibility(View.GONE);
...
//		int bottomMedium = R.drawable.herily_alertex_dlg_bg_bottom_dark;
		...
		//系统默认的背景
		int fullDark = a.getResourceId(R.styleable.HerilyAlertDialog_fullDark,
				R.drawable.herily_alertex_dlg_bg_full_dark);
		int topDark = a.getResourceId(R.styleable.HerilyAlertDialog_topDark,
				R.drawable.herily_alertex_dlg_bg_top_dark);
		int centerDark = a.getResourceId(R.styleable.HerilyAlertDialog_centerDark,
				R.drawable.herily_alertex_dlg_bg_center_dark);
		int bottomDark = a.getResourceId(R.styleable.HerilyAlertDialog_bottomDark,
				R.drawable.herily_alertex_dlg_bg_bottom_dark);
		int fullBright = a.getResourceId(R.styleable.HerilyAlertDialog_fullBright,
				R.drawable.herily_alertex_dlg_bg_full_dark);
		int topBright = a.getResourceId(R.styleable.HerilyAlertDialog_topBright,
				R.drawable.herily_alertex_dlg_bg_top_dark);
		int centerBright = a.getResourceId(R.styleable.HerilyAlertDialog_centerBright,
				R.drawable.herily_alertex_dlg_bg_center_dark);
		int bottomBright = a.getResourceId(R.styleable.HerilyAlertDialog_bottomBright,
				R.drawable.herily_alertex_dlg_bg_bottom_dark);
		int bottomMedium = a.getResourceId(R.styleable.HerilyAlertDialog_bottomMedium,
				R.drawable.herily_alertex_dlg_bg_bottom_dark);
	...
	public static class AlertParams {
		...
		public boolean mIsMultiCheckedEnable = true;
		...
		private void createListView(final SysAlertController dialog) {
			...
			if (mIsMultiChoice) {
				if (mCursor == null) {
                        adapter = new ArrayAdapter<CharSequence>(mContext, R.layout.dlg_select_multichoice,
				...
				if (mCursor == null) {
                     adapter = (mAdapter != null) ? mAdapter : new ArrayAdapter<CharSequence>(mContext,
							layout, android.R.id.text1, mItems);
				} else {
			...
			/*
			 * Don't directly set the adapter on the ListView as we might want to add
			 * a footer to the ListView later.
			 */
			...
			listView.setEnabled(mIsMultiCheckedEnable);
